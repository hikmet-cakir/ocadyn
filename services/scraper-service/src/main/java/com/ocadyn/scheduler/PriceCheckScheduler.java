        ...
        for (ActiveProductResponse product : activeProducts) {
            ...
            try {
                ...
                if (thresholdAlert) {
                    ...
                    sendNotification(product, update, type, NotificationDeliveryPolicy.buildThresholdMessage(update));
                }
                ...
                if (settings.channels().email()) {
                    log.info("Email notification requested for product {} (delivery not yet implemented)", product.id());
                }
